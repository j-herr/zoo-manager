...
public class GiftShopEmp
{
   ...
   public void addToInventory(Gift gift)
   {
   ...
   public boolean removeFromInventory(Gift gift)
   {
      ...
      for (int i = 0; i < Inventory.inStock.size(); i++)
      {
         g = Inventory.inStock.get(i);
         name2 = g.getGiftName();
         if (name1.equals(name2))
         {
            Inventory.inStock.remove(i);
            confirmed = true;
            return confirmed;
         }
         else
   ...
   //displays current inventory with prices from array in Inventory class
